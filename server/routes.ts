...
- 造語OK（ただし意味が通じること）
- 末尾単語がNG単語リストに記載されている単語は生成してはいけない。このバッチ内で同じ末尾単語を使用してはいけない。
- 生成後、上記の絶対ルールを全てのワードが守っているか検証。守られていないワードは削除せよ。
- 生成されたワードで同じ単語を使用している場合は「より強烈なパンチライン」なワードを残し、それ以外は削除すること。ひらがな・カタカナ・漢字、表現が違えど同じ言葉であった場合も同様。${level < 5 ? "\n- 放送禁止用語、差別用語、商標名（IP）を使用している場合は削除せよ。" : ""}
...
※必ず「/」の後にひらがな読みを書き、(romaji)を付けること`;
      ...
      const bannedTermsRule = level < 5
        ? "\n5. 放送禁止用語・差別用語・商標名（IP・キャラクター名等）を含む → 削除"
        : "";
      const step2Prompt = `【タスク】以下のディスワード一覧を絶対ルールで厳格に検査し、合格ワードのみ出力せよ。

【削除基準（以下に1つでも該当するワードは全て削除）】
1. ひらがな換算で4文字未満または11文字以上
2. 小学生にわからない言葉（難解な漢語・専門用語・マイナーな慣用句）
3. 【体言止め違反】名詞・名詞句以外で終わるワード:
   - 助詞終わり: 〜な/〜だ/〜わ/〜よ/〜ね/〜を/〜は/〜が/〜に/〜で/〜と/〜も/〜か/〜ぞ/〜ぜ
   - 助動詞終わり: 〜てる/〜てた/〜てた/〜です/〜ます/〜した/〜だろ/〜だぜ/〜だよ/〜だな
   - 動詞活用形終わり: 〜する/〜いる/〜れる/〜せる/〜られる/〜させる
   - 形容詞語尾終わり: 語尾が「い」（ただし名詞として確立している場合を除く）
4. 関西弁・方言語尾: やな/やわ/やろ/やで/やん/ねん/やんか/やんな/わな/じゃな/じゃろ/っちゃ/わい${bannedTermsRule}
6. 意味が通じない造語
7. 攻撃・批判・挑発・煽りでない（褒め言葉・ポジティブ表現）

【重複処理】
同じ末尾単語（例:「顔」「野郎」「ゴミ」等）を使うワードが複数ある場合、最も強烈なパンチラインのワードを1個だけ残し他を削除せよ。ひらがな・カタカナ・漢字など表記が違っても同じ意味・同じ末尾単語の場合も同様。
